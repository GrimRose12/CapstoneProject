...
def start():
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        f1 = executor.submit(detectRain)
        f2 = executor.submit(detectLevel)
        rainDepth, rainStatus = f1.result()
        level = f2.result()
        in_level = level[-1]
        lastDepth = rainDepth[-1]
    
    time = list(range(1,detectDuration+1))
    comp.createCSV(time, level, rainDepth)
    """
